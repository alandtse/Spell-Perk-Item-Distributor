...
namespace Lookup
{
	namespace detail
	{
		inline bool formID_to_form(RE::TESDataHandler* a_dataHandler, const FormIDPairVec& a_formIDVec, FormVec& a_formVec, std::string a_path)
		{
	...
	void get_forms(RE::TESDataHandler* a_dataHandler, const std::string& a_type, const INIDataVec& a_INIDataVec, FormDataVec<Form>& a_formDataVec)
	{
		...
		for (auto& [formIDPair_ini, strings_ini, filterIDs_ini, level_ini, gender_ini, itemCount_ini, chance_ini, path] : a_INIDataVec) {
			...
			for (std::uint32_t i = 0; i < 3; i++) {
				if (!detail::formID_to_form(a_dataHandler, filterIDs_ini[i], filterForms[i], path)) {
					invalidEntry = true;
					break;
				}
			}
